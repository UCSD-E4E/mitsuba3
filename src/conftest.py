...
for variant in mi.variants():
    ...
    if variant.startswith("metal") and not dr.has_backend(dr.JitBackend.Metal):
        continue
    if variant.startswith("hip") and not dr.has_backend(dr.JitBackend.HIP):
        continue
...
all_possible_variants = ["scalar" + s for s in suffix_variants] + [
    a + b + c for a in ["llvm", "cuda", "metal", "hip"] for b in ["", "_ad"] for c in suffix_variants]
